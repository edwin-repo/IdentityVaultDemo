...
export class SettingsService {
  ...
  async store(settings: { useBiometrics: boolean; usePasscode: boolean; useSecureStorageMode: boolean }) {
    if (settings.useBiometrics !== undefined) {
      this.storage.set(this.keys.biometrics, settings.useBiometrics).catch(err => {console.log('biostore'+err)});
    }
    if (settings.usePasscode !== undefined) {
      this.storage.set(this.keys.passcode, settings.usePasscode).catch(err => {console.log('biostore'+err)});
    }
    if (settings.useSecureStorageMode !== undefined) {
      this.storage.set(this.keys.secureStorageMode, settings.useSecureStorageMode).catch(err => {console.log('biostore'+err)});
    }
